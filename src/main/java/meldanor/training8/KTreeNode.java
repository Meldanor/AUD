 ...
 * <p>
 * 
 * The implementation supports an arbitrary number of children/keys for each
 * node, which are stored as entries ({@link Entry}) in a {@link Vector}.
 * <p>
 * 
 * Nodes support searching for an entry in the node and its subtrees, as
 * inserting an entry into a node/subtree as well as splitting and merging
 * nodes.
 * <p>
 * 
 * <ul>
 * 
 * <li>{@link #split} splits a node in two halves and creates a new tree rooted
 * by a 2-node, which stores the partitioning key.</li>
 * 
 ...
 * </ul>
 * 
 * Note that there are {@code k-1} keys for {@code k=}{@link #getK} children. The
 * node always stores {@code getK()} entries where the leftmost (0-th) key is
 * {@code null} by definition.
 * <p>
 * 
 * Different types of trees would use different strategies for splitting and
 * merging. This way, {@code KTreeNode} serves as a basis for implementation of,
 * e.g., 2-3-4 trees or B-trees. (The implementation is not efficient, similar
 * notes as for {@link BinarySearchTree} apply)
 * <p>
 * 
 ...
 * available.
 * 
 * Many method rely on correct input arguments. The implementation uses a lot of
 * assertions but hardly explicit error checking (for the correct-use cases)!
 * Make sure to run it with {@code "-ea"} when testing your code!
 */
public class KTreeNode<Key extends Comparable<Key>> implements Graphvizable, GraphvizDecorable {

    /**
     * Entry in a {@code KTreeNode} with reference to key and left child.
     * <p>
    ...
    /**
     * Compares keys where {@code null} keys refer to smallest key.
     * <p>
     */
    ...
    /**
     * Create new 1-node without key or children. This is intended mainly trooto
     * construct a root of an empty tree.
     */
    public KTreeNode() {
     ...
     * 
     * @param key
     * @return key instance found in tree (reference may differ from {@code key}
     *         !) or {@code null}
     */
     ...
     * 
     * @param child
     *            search for this node in the list of childs
    ...
    /**
     * Insert key in this node.
     * <p>
     * 
     * @param key
     *            to be inserted
     * @param k
     *            insert position, <b>must be correct</b> such that the order of
     *            entries_ is not violated (checked by assertions only)
    ...
    /**
     * Split this node at position k. After splitting, the node represents the
     * new tree with 2-node as root storing key at insertion position, the left
     * and right children store the range of keys left and right of k
     * 
     * @param k
     *            split position
     */
    ...
    /**
     * Merge child k into this node (pull child up).
     * <p>
    ...
    private String treeToDot(KTreeNode<Key> parent) {
        ...
        if (parent != null) { // edge is identified by "lower" node
            dot += " \"" + parent.dotRef() + "\" -- \"" + dotRef() + "\" ["; // no
                                                                             // edge
                                                                             // //
                                                                             // TODO
                                                                             // checkConsistency()labels
                                                                             // currently
