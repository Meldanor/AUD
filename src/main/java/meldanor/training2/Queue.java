...
/**
 * Impementation of AbstractQueue as a (dynamically resized) circular buffer
 * based on array.
 */
//@<queue:class
public class Queue<T> extends AbstractQueue<T> {
    ...
    /**
     * Compute real index (modulo buffer size). Make sure we can handle
     * "slighly negative" indices correctly (mind behaviour of %-Operator).
     */
    // @<queue:dequeue
    int wrap(int index) {
